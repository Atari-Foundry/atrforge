...
static void show_usage(void)
{
    printf("Usage: %s [options] <output_atr> [+attributes] <file_1> [... <file_n>]\n"
           "Options:\n"
           "\t-a\tAdd files to existing ATR image (instead of creating new).\n"
           ...
           "\t--to-atascii\tConvert files from UTF8 to ATASCII when adding to ATR.\n"
           "\t-h\tShow this help.\n"
...
static void write_atr(const char *out, const uint8_t *data, int ssec, int nsec)
{
    ...
    if( nsec > 3 )
    {
        // Check: ssec * (nsec - 3) + 128 * 3
        if( (nsec - 3) > INT_MAX / ssec )
            show_error("image size calculation overflow");
        int part1 = ssec * (nsec - 3);
        int part2 = 128 * 3;
        if( part1 > INT_MAX - part2 )
            show_error("image size calculation overflow");
        size = part1 + part2;
    }
    else
    ...
    for( int i = 0; i < nsec; i++ )
    {
        ...
        if( fwrite(data + ssec * i, write_size, 1, f) != 1 )
        {
            fclose(f);
            show_error("can't write sector %d to output file '%s': %s", i + 1, out,
                       strerror(errno));
        }
    }
    if( 0 != fclose(f) )
        show_error("can't write output file '%s': %s", out, strerror(errno));
}
...
static int image_size(int nsec, int ssec)
{
    ...
    if( ssec == 256 )
    {
        ...
        {
            // Check overflow: nsec * ssec
            if( nsec > INT_MAX / ssec )
                return -1;
            int total = nsec * ssec;
            int padding = 3 * 128;
            if( total < padding )
...
int main(int argc, char **argv)
{
    ...
    int add_mode       = 0;                      // Add to existing ATR
    int to_atascii     = 0;                      // Convert UTF8 to ATASCII
    ...
    // Enable conversion if requested
    if( to_atascii )
        flist_set_convert_utf8_to_atascii(1);

    // Check if adding to existing file
    if( add_mode )
    {
        struct stat st;
        if( stat(out, &st) != 0 )
            show_error("file '%s' does not exist (use without -a to create new)", out);
        if( darray_len(&flist) <= 1 )
            show_error("no files to add");
        // Remove the main directory entry for adding
        struct afile *main_dir = darray_i(&flist, 0);
        // Create new list with only files to add
        file_list new_files;
        darray_init(new_files, 1);
        struct afile **ptr;
        darray_foreach(ptr, &flist)
        {
            struct afile *af = *ptr;
            if( af != main_dir )
                darray_add(&new_files, af);
        }
        int ret = modatr_add_files(out, &new_files);
        darray_delete(flist);
        darray_delete(new_files);
        return ret;
    }
